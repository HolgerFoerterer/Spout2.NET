...
namespace Spout.Std
{
    ...
    // DEBUG: 
    // DEBUG:     static_assert(!is_array_v<_Elem> && is_trivially_copyable_v<_Elem> && is_trivially_default_constructible_v<_Elem>
    // DEBUG:                       && is_standard_layout_v<_Elem>,
    // DEBUG:         "The character type of basic_string must be a non-array trivially copyable standard-layout type T where "
    // DEBUG:         "is_trivially_default_constructible_v<T> is true. See N5001 [strings.general]/1.");
    // DEBUG: 
    // DEBUG: public:
    ...
    // DEBUG:     // N5001 [strings.general]/1 says _Elem must be trivially copyable standard-layout, so memcpy is safe.
    ...
    // DEBUG:     static constexpr bool _Can_memcpy_val =
    // DEBUG:         _Is_specialization_v<_Traits, char_traits> && is_trivially_copyable_v<pointer>;
    ...
    // DEBUG:         if constexpr (!_Disable_ASan_container_annotations_for_allocator<allocator_type>) {
    // DEBUG: #if _HAS_CXX20
    ...
    // DEBUG:                 const size_type _Count) _STATIC_LAMBDA {
    ...
    // DEBUG:                 const _Elem _Ch) _STATIC_LAMBDA {
    // DEBUG:                 _Traits::copy(_New_ptr, _Old_ptr, _Old_size);
    ...
    // DEBUG:             _Count,
    // DEBUG:             [](_Elem* const _New_ptr, const size_type _Count, const _Elem* const _Ptr) _STATIC_LAMBDA {
    ...
    // DEBUG:         return _Reallocate_for(
    // DEBUG:             _Count,
    // DEBUG:             [](_Elem* const _New_ptr, const size_type _Count, const _Elem _Ch) _STATIC_LAMBDA {
    ...
    // DEBUG:                 const _Elem* const _Ptr, const size_type _Count) _STATIC_LAMBDA {
    ...
    // DEBUG:             _Count,
    // DEBUG:             [](_Elem* const _New_ptr, const _Elem* const _Old_ptr, const size_type _Old_size, const size_type _Off,
    // DEBUG:                 const size_type _Count, const _Elem _Ch) _STATIC_LAMBDA {
    // DEBUG:                 _Traits::copy(_New_ptr, _Old_ptr, _Off);
    ...
    // DEBUG:             _Growth,
    // DEBUG:             [](_Elem* const _New_ptr, const _Elem* const _Old_ptr, const size_type _Old_size, const size_type _Off,
    // DEBUG:                 const size_type _Nx, const _Elem* const _Ptr, const size_type _Count) _STATIC_LAMBDA {
    // DEBUG:                 _Traits::copy(_New_ptr, _Old_ptr, _Off);
    // DEBUG:                 _Traits::copy(_New_ptr + _Off, _Ptr, _Count);
    ...
    // DEBUG:             _Count - _Nx,
    // DEBUG:             [](_Elem* const _New_ptr, const _Elem* const _Old_ptr, const size_type _Old_size, const size_type _Off,
    // DEBUG:                 const size_type _Nx, const size_type _Count, const _Elem _Ch) _STATIC_LAMBDA {
    // DEBUG:                 _Traits::copy(_New_ptr, _Old_ptr, _Off);
    // DEBUG:                 _Traits::assign(_New_ptr + _Off, _Count, _Ch);
    // DEBUG:                 _Traits::copy(_New_ptr + _Off + _Count, _Old_ptr + _Off + _Nx, _Old_size - _Nx - _Off + 1);
    // DEBUG:             },
    ...
    // DEBUG: #if _MSVC_STL_HARDENING_BASIC_STRING || _ITERATOR_DEBUG_LEVEL != 0
    ...
    // DEBUG: 
    // DEBUG: #if _MSVC_STL_HARDENING_BASIC_STRING || _ITERATOR_DEBUG_LEVEL != 0
    // DEBUG:         _STL_VERIFY(_Old_size != 0, "pop_back() called on empty string");
    // DEBUG: #endif
    // DEBUG: 
    ...
    // DEBUG:                     _STATIC_LAMBDA { _Traits::copy(_New_ptr, _Old_ptr, _Old_size + 1); });
    // DEBUG:         } else {
    ...
    // DEBUG:                 _STATIC_LAMBDA { _Traits::copy(_New_ptr, _Old_ptr, _Old_size + 1); });
    // DEBUG: 
    // DEBUG:         // `_Reallocate_grow_by` calls `_ASAN_STRING_CREATE` assuming that the string
    // DEBUG:         // has size (initialized memory) equal to its new capacity (allocated memory).
    // DEBUG:         // This is not true for the `reserve` method, so we modify the ASan annotation.
    // DEBUG:         _ASAN_STRING_MODIFY(*this, _Mypair._Myval2._Mysize, _Old_size);
...
// DEBUG: 
// DEBUG:     DXGI_FORMAT_A4B4G4R4_UNORM                          = 191,
// DEBUG: 
// DEBUG: 
